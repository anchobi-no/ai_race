...
def init_inference():
    ...
    model = DQN(240, 320, DISCRETIZATION)
...
bridge = CvBridge()
twist = Twist()
pre_action = torch.Tensor([[0.0]]).to(device)
...
def set_throttle_steer(data):
    ...
    outputs = model(image, pre_action)

    outputs_np = outputs.to('cpu').detach().numpy().copy()

    output = np.argmax(outputs_np, axis=1)
    print(output)
    pre_action =  torch.Tensor(np.array([output])).to(device)
...
def parse_args():
    ...
    arg_parser.add_argument("--pretrained_model", type=str, default='/home/sakamoto/catkin_ws2/src/ai_race/ai_race/reinforcement_learning/model_weight/dqn_20210402.pth')
    #arg_parser.add_argument("--trt_model", type=str, default='/home/jetson/catkin_ws/src/ai_race/ai_race/reinforcement_learning/trt_model/dqn_20210113_trt.pth')
