...
Transition = namedtuple(
    'Transition', ('state', 'action', 'next_state', 'reward', 'pre_act')
)
...
class Memory:
    ...
    def push(self, state, action, state_next, reward, pre_act):
        ...
        self.memory[self.index] = Transition(state, action, state_next, reward, pre_act)
...
class Brain:
    ...
    def replay(self):
        ...
        # 2. ミニバッチの作成
        self.batch, self.state_batch, self.action_batch, self.reward_batch, self.non_final_next_states, self.pre_action_batch, self.non_final_pre_action = self.make_minibatch()
    ...
    def make_minibatch(self):
        ...
        pre_action_batch = torch.cat(batch.pre_act)
        #print('act_size:', torch.Size(action_batch))
        #print('pre_act_size:', torch.Size(pre_action_batch))

        non_final_next_states = torch.cat([s for s in batch.next_state if s is not None])
        non_final_pre_action = torch.cat([a for a, s in zip(batch.pre_act, batch.next_state) if s is not None])

        return batch, state_batch, action_batch, reward_batch, non_final_next_states, pre_action_batch, non_final_pre_action
    ...
    def get_expected_state_action_values(self):
        ...
        self.state_action_values = self.main_q_network(self.state_batch, self.pre_action_batch).gather(1, self.action_batch)
        ...
        a_m[non_final_mask] = self.main_q_network(
            self.non_final_next_states, self.non_final_pre_action).detach().max(1)[1]
        ...
        next_state_values[non_final_mask] = self.target_q_network(self.non_final_next_states, self.non_final_pre_action).gather(1, a_m_non_final_next_states).detach().squeeze()
    ...
    def decide_action(self, state, pre_act, epsilon):
        '''現在の状態に応じて、行動を決定する'''
        ...
        if epsilon <= np.random.uniform(0, 1):
            ...
            with torch.no_grad():
                output = self.main_q_network(state, pre_act)
                if self.debug:
...
class Agent:
    ...
    def get_action(self, state, pre_act, epsilon):
        '''行動を決定する'''
        action = self.brain.decide_action(state, pre_act, epsilon)
        return action

    def memorize(self, state, action, state_next, reward, pre_act):
        '''memoryオブジェクトに、state, action, state_next, rewardの内容を保存する'''
        self.brain.memory.push(state, action, state_next, reward, pre_act)
